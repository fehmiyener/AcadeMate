...
let getCreateLessonPage = (req, res) => {
    return res.render("mrlessons.ejs");
};
...
let createNewLesson = async (req, res) => {
    ...
    try {
        ...
        await mrlessonService.createNewLesson(lessonData, req.user.id);
        return res.redirect("/mrlessons"); // Redirect to profile page after lesson creation
    } catch (error) {
        console.error(error);
